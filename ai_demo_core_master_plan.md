...
6. **Phase 3.5:** Trial Portal 分離（お客様 `/trial` · 管理 `/admin/trial`）— デモ量産の共通導線
...
お客様向け取得（`/trial`）と管理者画面（`/admin/trial`）を分離し、今後のデモ横展開の URL 契約を固定する。

### 完了条件

- [x] `VITE_TRIAL_PORTAL_URL` → `/trial`（`/admin/trial` をお客様導線にしない）
- [x] Studio 公開 `/trial` + `POST /api/trial/issue`
- [x] `?demo=` / `?return=` の受け取り（文言・デモに戻る）
- [ ] 本番 `demo.axeon.jp` デプロイとポータル URL 確定
- [ ] 各新規デモは catalog エントリ追加だけで誘導できる
...
**Phase 3.5（進行中）：Trial Portal 分離 — デモ量産前提の Platform 導線**
...
として進める。Studio（将来 `demo.axeon.jp`）は次の二面を持つ。

1. それ自体が動く AI チャットデモ（`/`）
2. 全デモ共通の Trial インフラ（`/trial` 取得 · `/admin/trial` 管理 · `/api/trial/*`）

各デモは基本的に次だけで横展開する。

1. `ExperienceModeBar` を置く
2. 「体験コードを取得」→ `VITE_TRIAL_PORTAL_URL`（`/trial?demo=…&return=…`）
3. 共通 API でコード検証する

## 現在の作業

- product_flow: Access Mode + ExperienceModeBar + `VITE_TRIAL_PORTAL_URL`
- Studio: 公開 `/trial` + `POST /api/trial/issue`（IP レート制限）
- `/admin/trial` は管理者専用（一覧・失効）

詳細: `product_flow/docs/PHASE3_FULL_HANDOFF.md`

## 次の作業

1. Phase 3 手動受け入れ（BYOK / Trial / 文書投入）
2. Phase 3.5 仕上げ: 本番 `demo.axeon.jp` デプロイ、ポータル URL 確定
3. Phase 4 DDデモへ組み込み（同じ `/trial` 導線）
4. 以降: OCR / 音声 / 写真 / 報告書へ ExperienceModeBar 横展開
...
- [ ] Phase 2動作確認
- [x] Phase 3 ISOデモへ導入
- [ ] Phase 3.5 Trial Portal 分離（`/trial` 公開 · `/admin/trial` 管理）
- [ ] Phase 4 DDデモへ導入
- [ ] Phase 5 正式パッケージ化判断
...
**体験コードの「取得ページ」と「管理画面」を分離する。お客様導線は `/trial`、管理者は `/admin/trial`。**

- 各デモの環境変数は `VITE_TRIAL_PORTAL_URL`（例: `https://demo.axeon.jp/trial`）
- `/admin/trial` をお客様に渡さない
- デモ量産時は `?demo=` / `?return=` で取得元を引き継ぐ

理由: デモが増えるほど共通ポータルの価値が上がり、管理シークレット付き画面を公開導線にするとプラットフォームではなく社内ツールに見えるため。
