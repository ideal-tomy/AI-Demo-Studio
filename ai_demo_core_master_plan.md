...
6. **Phase 3.5:** Trial Portal（共通飛ばし先 `/admin/trial`）— デモ量産の共通導線
7. **Phase 4:** DDデモへ組み込み、フォーム型でも再利用できることを検証する
8. **Phase 5:** 実績を踏まえて正式な共通基盤（Platform）へ昇格する
...
9. **体験コードの取得場所は全デモで共通1箇所**とする。各デモに取得UIを複製せず、指定ポータルへリンクして取得する（デモ量産の前提）
...
（実装完了。詳細受け入れは `product_flow/docs/PHASE3_FULL_HANDOFF.md`）

---

## Phase 3.5：Trial Portal 分離（デモ量産の共通導線）

### 目的

お客様向け `/trial` と管理 `/admin/trial` の画面は存在するが、**デモ量産時の共通飛ばし先は `/admin/trial` に統一**する。

**デモ量産の前提（確定）:** 体験コードの取得は「デモごと」ではなく **Platform 側の共通1箇所（`/admin/trial`）** で行う。新規デモは取得画面を自前実装せず、`VITE_TRIAL_PORTAL_URL`（既定で `/admin/trial`）へ飛ばして取得する。発行・制限・失効も同じ基盤を共有する。

### 完了条件

- [x] `VITE_TRIAL_PORTAL_URL` で各デモから共通ポータルへ誘導できる
- [x] Studio 公開 `/trial` + `POST /api/trial/issue`（説明ページとして維持）
- [x] `?demo=` / `?return=` の受け取り（文言・デモに戻る）
- [x] ポータル URL 契約確定（現行正・共通飛ばし先）: `https://ai-demo-studio-lime.vercel.app/admin/trial`
- [x] catalog エントリ追加で誘導できる（`iso-chat` · `dd-diagnosis` 等）
- [ ] 将来: 本番 `demo.axeon.jp` へ DNS 差し替え（スコープ外・URL 差し替えのみ）
...
- [x] 同じCoreでチャット型とフォーム型を動かせる（product_flow · dd_demo）
- [x] 構造化JSON出力を扱える（DD Output Adapter）
- [x] DD固有の入力・出力UIを保ったままAI基盤を再利用できる（`/` 演出 + `/ai` 診断）
...
本プロジェクトは「共通 Core ライブラリ」に閉じず、

> **複数の AI デモを、統一された体験モード・制限・認証で提供する AI Demo Platform**

として進める。Studio（現行 `ai-demo-studio-lime.vercel.app`、将来 `demo.axeon.jp`）は次の二面を持つ。

1. それ自体が動く AI チャットデモ（`/`）
2. 全デモ共通の Trial インフラ（共通飛ばし先 `/admin/trial` · `/api/trial/*`）

### 体験コード取得の方針（デモ量産前提）

- **取得場所は1つ。** デモごとに体験コード発行画面を持たない。
- 各デモの「体験コードを取得」は、**常に `/admin/trial`**（または同等の指定 URL）へリンクする。
- **現時点の共通取得場所（正）:** [https://ai-demo-studio-lime.vercel.app/admin/trial](https://ai-demo-studio-lime.vercel.app/admin/trial)
- `/trial` 説明ページは残してもよいが、デモからの飛ばし先には使わない。
- 将来 `demo.axeon.jp` へ DNS 差し替え時も、パスは `/admin/trial` のまま `VITE_TRIAL_PORTAL_URL` のホストだけ差し替える。
...
**体験コードの共通飛ばし先は `/admin/trial` に統一する。**

- 各デモの環境変数は `VITE_TRIAL_PORTAL_URL`（例: `https://ai-demo-studio-lime.vercel.app/admin/trial`）
- パスが `/` または `/trial` の場合も実装側で `/admin/trial` に正規化する
- デモ量産時は `?demo=` / `?return=` で取得元を引き継ぐ
- `/trial` 説明ページは任意の補助導線として残してよいが、デモ UI からはリンクしない

理由: 発行・案内・一覧・失効を1画面に集約し、デモ追加時の導線を単純にするため（当面の運用方針）。

## Decision 019

**設計思想を「AI Demo Core」単体ではなく「AI Demo Platform」として扱う。**

Studio は (1) 動くチャットデモ かつ (2) 全デモ共通の Trial / Access Mode 基盤。  
横展開の最小単位は ExperienceModeBar + ポータルリンク + 共通検証 API。

## Decision 020

**体験コード取得は全デモで共通1箇所 `/admin/trial` に統一する。**

方針:

- 新規デモを量産する際、体験コード発行・案内UIをデモ側に複製しない
- 各デモは「体験コードで接続」入力を持ち、「取得」は `/admin/trial` へリンクする
- 現時点の指定先: [https://ai-demo-studio-lime.vercel.app/admin/trial](https://ai-demo-studio-lime.vercel.app/admin/trial)
- 本番 URL 確定後も、差し替えるのはホストのみ。パスは `/admin/trial` を維持する

理由: 共通基盤を先に整え、デモ追加コストを「UI埋め込み + ポータル誘導 + 検証」に閉じるため。

## Decision 021

**Phase 4 は既存簿外債務演出デモを壊さず、フォーム型 AI 診断を `/ai` として併存させる。**

- catalog id: `dd-diagnosis`
- Core / Trial は product_flow と同系統の vendor コピー
